# TxtFile.pm
# MIMAS Repository Txt File Class
#
#######################################################
...
package MIMAS::DB::Repository::TxtFile;
...
sub samples {
    my $self = shift;
    $self->{samples} = $self->adaptor->area->SampleAdaptor->select_all_by_txtID($self->dbID) unless defined $self->{samples};
    return $self->{samples};
}
